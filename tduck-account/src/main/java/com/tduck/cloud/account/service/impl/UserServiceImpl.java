...
public class UserServiceImpl extends ServiceImpl<UserMapper, UserEntity> implements UserService {
    ...
    @Override
    public Result accountFBLogin(String userid,String ip,String fbToken) {
        UserEntity userEntity = getUserByFBUserId(userid);
        if (ObjectUtil.isNull(userEntity)) {
            return Result.failed("未授权，请重新授权");
        }
        return Result.success(getLoginFBResult(userEntity,
                AccountChannelEnum.FB,
                ip,fbToken));
    }
     ...
     */
    @Override
    public LoginUserVO getLoginFBResult(UserEntity userEntity, AccountChannelEnum channel, String requestIp,String FBToken) {
        userEntity.setLastLoginIp(requestIp);
        userEntity.setLastLoginChannel(channel);
        userEntity.setLastLoginTime(LocalDateTime.now());
        this.updateById(userEntity);
        String token = jwtUtils.generateToken(userEntity.getId());
        return new LoginUserVO(userEntity.getAvatar(), userEntity.getName(), token,FBToken);
    }
    ...
    @Override
    public UserEntity getUserByFBUserId(final String FBUserId) {
        return this.getOne(Wrappers.<UserEntity>lambdaQuery().eq(UserEntity::getFbUser, FBUserId));
    }
